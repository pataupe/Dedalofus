...
- **Catégories de filtres pour les breloques** (Dégâts, Mobilité, Soin/Protection, Entrave, Bonus PA/PO, Bonus divers, Breloques boss) : les boutons existent déjà sur `/breloques` ("bientôt actifs") et la donnée existe maintenant (`Breloque.tag`, importée depuis `DEDALE - BRELOQUES_NEW.csv`, ex: `"Soin - Protection"`, ou composée `"Breloque boss + Dégâts"` sur certaines Unique) — mais le filtre n'est **pas encore branché** côté `breloquesController.js`/`BreloqueListPage.jsx` (hors scope du chantier Boosts, pas demandé). Prêt à câbler rapidement le jour où c'est demandé : il suffirait d'un filtre `LIKE` sur `tag`, même principe que le filtre `elements` des sorts (plusieurs tags possibles par breloque).
