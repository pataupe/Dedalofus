...
- **Explicitement hors scope de ce chantier** (décision du porteur de projet, à faire dans un 2ᵉ temps une fois cet onglet validé) : les bonus configurés ici **n'affectent pas encore** `calculerStatsPersonnage`/`calculerDegats` — c'est un pur choix d'affichage/sauvegarde pour l'instant. Restent à faire : brancher réellement les bonus dans le calcul de dégâts (règles de cumul déjà données par le porteur de projet, voir "Points encore en suspens"), + 2 nouveaux toggles sur l'onglet Sorts (Distance/Mêlée, par défaut Distance ; Dégâts de base/calculés, par défaut calculés).
...
- **Onglet "Boosts breloques" pas encore branché au calcul de dégâts** (décision explicite du porteur de projet, cf. "Onglet Boosts breloques" ci-dessus — livré d'abord seul pour validation). Règles de cumul des multiplicateurs de dommages **déjà données et à respecter à la lettre** quand ce sera fait :
  - Chaque multiplicateur de dommages a un **type** : `dommages finaux`, `dommages finaux distance`, ou `dommages finaux mêlée` (voir le préfixe renvoyé par `parseValeurBoost`, `server/logic/boosts.js`).
  - Deux multiplicateurs du **même type exact** → leurs bonus (partie après le "x", ex: x1.15 = +15%) **s'additionnent**. Exemple : "dommages finaux distance" x1.15 (Tireur d'élite) + x1.6 (Sentinelle) = x1.75 (et non x1.84).
  - Deux multiplicateurs de **types différents** → ils se **multiplient** entre eux tels quels. Exemple : "dommages finaux" x1.55 × "dommages finaux distance" x1.15 = x1.7825.
  - Une attaque est **toujours soit distance soit mêlée**, jamais les deux à la fois — d'où le nouveau toggle Distance/Mêlée prévu sur l'onglet Sorts (voir point suivant), qui détermine quels multiplicateurs "distance"/"mêlée" s'appliquent à un calcul donné (ceux de l'autre catégorie ne comptent pas).
  - Restent aussi à traiter dans le calcul, une fois branché : les bonus "toggle" à effets composites non chiffrables en un seul nombre (ex: Embourbeur = Retrait PA/PM + Esquive PA/PM ; Forcené = % Coups Critiques + Dommages Critiques ; Hémophile = % PdV) — `parseValeurBoost` renvoie `valeur: null` pour ces textes-là actuellement, aucune décomposition en effets structurés individuels n'a encore été conçue.
- **2 nouveaux toggles prévus sur l'onglet Sorts** (pas encore faits, dépendent du branchement du calcul ci-dessus) : un toggle **Distance/Mêlée** (par défaut Distance) déterminant quels multiplicateurs de boost s'appliquent, placé entre les 3 boutons d'onglet (Équipement/Boosts breloques/Sorts) et la grille de cartes de sorts ; et à côté, un toggle **Dégâts de base/calculés** (par défaut calculés, comportement actuel) pour basculer l'affichage de `SortCard` entre les valeurs brutes du sort et le résultat de `calculerDegats`.
