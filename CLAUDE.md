...
- **Sourcing des images** des équipements : démarré (breloques + sorts, rang Novice seulement — voir "Premières images réelles" ci-dessus). Restent à faire : Expert/Maître α/ẞ pour les breloques, les autres rangs de sorts, les cubes, et les breuvages (toujours aucune donnée).
...
- **Fichiers sources ajoutés mais pas encore traités** : `data/DEDALE - SORTS EXPERT - DEDALE.csv`, `data/DEDALE - SORTS MAÎTRE α - DEDALE.csv`, `data/DEDALE - SORTS MAÎTRE ẞ - DEDALE.csv` — déposés par le porteur de projet (non commités à la fin de cette session), pas encore importés. À traiter : adapter/dupliquer `import-sorts.js` pour ces rangs (actuellement la base ne contient que des sorts `rang_evolution = 'Novice'`), en gardant le même retraitement des colonnes que pour le CSV Novice (voir section "Format des données sources").
